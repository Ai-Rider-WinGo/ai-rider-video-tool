...
const { getLicenseSigningKey } = require("./keychain");
...
function createLicenseStore({ filePath }) {
  ...
  function ensureStorage() {
    ...
    if (!fs.existsSync(resolvedPath)) {
      const initial = { licenses: [], signature: computeSignature([]) };
  ...
  function computeSignature(licenses) {
    let key;
    try { key = getLicenseSigningKey(); } catch { return ""; }
    if (!key) return "";
    const payload = JSON.stringify({ licenses });
    return crypto.createHmac("sha256", key).update(payload).digest("hex");
  }

  function verifySignature(db) {
    const stored = db.signature || "";
    if (!stored) {
      // First time: no signature — auto-sign (legacy data is trusted)
      return true;
    }
    const expected = computeSignature(db.licenses);
    if (!expected) return true; // Can't verify without key — trust existing data
  ...
  function readDb() {
    ...
    try {
      ...
      if (!verifySignature(parsed)) {
        throw new Error("licenses.json 签名验证失败——数据可能已被篡改。已重置为空白记录。");
      }
      return { licenses: parsed.licenses };
    } catch (error) {
      if (error.message && error.message.includes("签名验证失败")) {
        throw error;
      }
      return { licenses: [] };
    }
  ...
  function writeDb(db) {
    ensureStorage();
    db.signature = computeSignature(db.licenses);
